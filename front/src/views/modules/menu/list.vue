...
export default {
  data() {
	...
	hiddenMenuTables: ['yiqingjiance', 'wuyeshoufei'],
	menuList: [],
	roleName: '管理员',
	roleNameList: [],
	backMenuList: [],
  ...
  methods: {
	isHiddenMenuItem(item) {
		return !!(item && item.tableName && this.hiddenMenuTables.includes(item.tableName));
	},
				...
				for (var i in this.menuList) {
					...
					if (this.menuList[i].roleName == rn) {
						...
						for (var j in this.backMenuList) {
							...
							let visibleChildMenuList = childMenuList.filter(item => !this.isHiddenMenuItem(item));
							if (visibleChildMenuList.length > 0) {
								this.parentMenuNameList.push(parentMenuName);
								for (var k in visibleChildMenuList) {
									this.backChildMenuList.push({
										...visibleChildMenuList[k],
										parentMenuName
									});
								}
							} else if (childMenuList.length === 0) {
								this.parentMenuNameList.push(parentMenuName);
								let parentNode = {};
